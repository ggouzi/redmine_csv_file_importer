...
class ImporterController < ApplicationController
  ...
  def match
    ...
    # iip = ImportInProgress.find_or_create_by_user_id(User.current.id) (pour RAILS <=3)
	iip = ImportInProgress.find_or_create_by(user_id: User.current.id) # (pour RAILS=4)
	...
	render(:template => "importer/match_" + render_template)
  ...
  def result
	...
	if result_errors.size > 0
		...
		render(:template => "importer/result_" + render_template)
	end
  end
...
private
  ...
  def flash_message(type, text)
   ...
   def get_settings
    @settings = Setting.plugin_importer
  end
